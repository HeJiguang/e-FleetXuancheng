...
createApp({
    ...
    data() {
        return {
            ...
            departmentView: {
                ...
                error: null,
                data: [], // 用于存储从后端获取的部门列表
                dataLoaded: false, // 用于防止重复加载数据
                // (修复) 统一内部命名，将 'distance' 改为 'mileage'
                activeTab: 'mileage', 
                // (移除) 不再需要 showAll 状态
            },
            isLoading: true,
    ...
    computed: {
        ...
        // (新增) 为部门视图创建的计算属性
        departmentRankings() {
            if (!this.departmentView.data || this.departmentView.data.length === 0) {
                return { mileage: [], fuel: [], violations: [], maintenance: [] };
            }
            const data = [...this.departmentView.data];

            return {
                // (修复) 统一 key 为 'mileage'
                mileage: [...data].sort((a, b) => b.total_distance - a.total_distance),
                fuel: [...data].sort((a, b) => b.total_fuel - a.total_fuel),
                violations: [...data].sort((a, b) => b.violation_count - a.violation_count),
                maintenance: [...data].sort((a, b) => b.total_maintenance_cost - a.total_maintenance_cost)
            };
        }
    },
    ...
    methods: {
        ...
        navigateTo(view) {
            ...
            if (view === 'overview') {
                ...
                // (新增) 首次切换到部门视图时，加载其数据
                if (!this.departmentView.dataLoaded) {
        ...
        // (新增) 为部门总览页面创建的异步数据获取方法
        async a_fetchDepartmentData() {
            this.departmentView.isLoading = true;
            this.departmentView.error = null;

            // 注意：我们暂时先不加时间筛选，后续会统一加上
            const url = `http://127.0.0.1:5000/api/department/summary`;
            ...
            try {
                ...
                this.departmentView.data = await response.json();
                this.departmentView.dataLoaded = true; // 标记数据已成功加载
            } catch (err) {
                console.error('获取部门数据失败:', err);
                this.departmentView.error = '无法加载部门数据，请检查后端服务。';
            } finally {
                this.departmentView.isLoading = false;
                // (新增) 部门数据加载完成后，渲染初始图表
                this.$nextTick(() => {
        ...
        // (新增) 部门视图相关的方法
        changeDepartmentTab(tabId) {
            this.departmentView.activeTab = tabId;
            this.$nextTick(() => {
                this.renderDepartmentChart();
            });
        },

        // (移除) toggleRanking 和 getVisibleRanking 方法不再需要
        
        renderDepartmentChart() {
            const metric = this.departmentView.activeTab;
            if (!metric) return;

            const ranking = this.departmentRankings[metric];
            ...
            const chartData = {
                ...
                data: ranking.map(d => {
                    // (修复) 统一使用 'mileage'
                    if (metric === 'mileage') return d.total_distance;
                    if (metric === 'fuel') return d.total_fuel;
                    if (metric === 'violations') return d.violation_count;
                    if (metric === 'maintenance') return d.total_maintenance_cost;
                    return 0;
                })
            };
            ...
            this.renderChart(
                `department${metric.charAt(0).toUpperCase() + metric.slice(1)}Chart`,
                'bar',
                chartData,
                {
                    label: `各部门${labelMap[metric] || ''}对比`,
